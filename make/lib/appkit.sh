...
folder_library="\$folder_dpishit/lib/dpishit/appkit"
...
defines+=("-DDPISHIT_ERROR_LOG_DEBUG")
...
flags+=("-fPIC")
flags+=("-O2")
defines+=("-DDPISHIT_ERROR_LOG_MANUAL")
	;;
...
echo "folder_dpishit = $folder_dpishit"; \
...
{ \
echo "# copy headers"; \
echo "build \$folder_include/dpishit_appkit.h: \$"; \
echo "cp src/include/dpishit_appkit.h"; \
echo ""; \
} >> "$output/$ninja_file"

{ \
echo "build headers: phony \$"; \
echo "\$folder_include/dpishit_appkit.h"; \
echo ""; \
} >> "$output/$ninja_file"
